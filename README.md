...
Application containers publish no host port; public entry points go through Cloudflare Tunnel with HTTPS everywhere. Production and hors-production stacks are isolated (containers, networks, configuration, credentials, data, logs).
...
The software license does not grant rights to use the NOIOU name, logo or project identity in a way that suggests an unofficial fork or service is the official NOIOU project.
